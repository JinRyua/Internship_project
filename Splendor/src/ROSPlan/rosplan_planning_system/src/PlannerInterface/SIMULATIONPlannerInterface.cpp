#include "rosplan_planning_system/PlannerInterface/SIMULATIONPlannerInterface.h"
...
namespace KCL_rosplan {
	...
	SIMULATIONPlannerInterface::SIMULATIONPlannerInterface(ros::NodeHandle& nh)
	{
		node_handle = &nh;
        ...
        thread_size = thread::hardware_concurrency();   //get thread size
        thread_list.resize(thread_size);
	...
	SIMULATIONPlannerInterface::~SIMULATIONPlannerInterface()
	{
		delete plan_server;
	}
	 ...
	 */
	std::string SIMULATIONPlannerInterface::runCommand(std::string cmd) {
		std::string data;
		FILE *stream;
		char buffer[1000];
	 ...
	 */
	bool SIMULATIONPlannerInterface::runPlanner() {
    ...
    void SIMULATIONPlannerInterface::make_problem_for_csp(string& problem_instance, problem_info& problem){
    ...
    bool SIMULATIONPlannerInterface::check_endgame(problem_info& gameState) {  //check endgame
    ...
    double SIMULATIONPlannerInterface::minimax(problem_info& gameState, bool isMax, unsigned int depth, string character, double alpha, double beta) {
		//minimaxCout++;
        ...
        if(depth >= planning_depth || check_endgame(gameState)){  //set depth
            //cout<<"hi"<<endl;
            return simulation(gameState, character, SIMULATION_COUNT, 0);
        }
        ...
        vector<action> act_list = MakeActionList(gameState, playerChar);
    ...
    std::vector<action> SIMULATIONPlannerInterface::MakeActionList(problem_info& gameState, std::string playerChar){
    ...
    double SIMULATIONPlannerInterface::simulation(problem_info& gameState, std::string character, const int count, int depth) {     //simulation of gameState  -> return probabilty
        double timer = 0;
        if(depth==0){
        ...
        string playerChar = character;  //make name string
        string nextplayer;
        if(character == "player1")
            nextplayer = "ai1";
        else
        ...
        vector<action> possible_action_list = MakeActionList(gameState, character); //make possible act list
        int size = possible_action_list.size();


        if(depth == 0){ //start node of simulation
            double rate = 0;
            ...
            for(int j = 0;j<thread_size;j++){
                if (j == thread_size - 1) {
                    ...
                    thread_list[j] = thread([&](){StartSimulationByThread(j, possible_action_list, rate, gameState, playerChar, current_count, (count / thread_size) + current_count, depth);});
                    current_count += count / thread_size;
                }
            }
            ...
            double finish_timer = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
            
            cout<<"finish simulation : "<<(finish_timer-timer)/1000000000<<endl;
            cout<<rate/count<<endl;
            return rate / (double)count;
        } else {                                               //processing simulation
            int rand_num = rand() % size;                      //random
            if (possible_action_list[rand_num].action == 0) {  //3 coin
                three_coin(gameState, possible_action_list[rand_num].details, possible_action_list[rand_num].abandon, playerChar);
            } else if (possible_action_list[rand_num].action == 1) {  //2coin
                two_coin(gameState, possible_action_list[rand_num].details, playerChar);
            } else if (possible_action_list[rand_num].action == 2) {  //buy
                buy_card(gameState, possible_action_list[rand_num].details, playerChar);
            } else return LOSE;
            

            return simulation(gameState, nextplayer, count, depth + 1);
        }
    }

    void SIMULATIONPlannerInterface::StartSimulationByThread(int number, vector<action>& possible_action_list, double& rate, problem_info& gameState, string& playerChar, const int start, const int end, int depth) {
          //start thread
          double timer = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
          for (int i = start; i < end; i++) {
                //cout<<i<<endl;
                int rand_num = rand() % possible_action_list.size();  //random
                problem_info old_gameState;
                old_gameState = gameState;     //backup

                // set board for calling simulation()
                if (possible_action_list[rand_num].action == 0) {  //3 coin
                    three_coin(old_gameState, possible_action_list[rand_num].details, possible_action_list[rand_num].abandon, playerChar);
                } else if (possible_action_list[rand_num].action == 1) {  //2coin
                    two_coin(old_gameState, possible_action_list[rand_num].details, playerChar);
                } else if (possible_action_list[rand_num].action == 2) {  //buy   
                    buy_card(old_gameState, possible_action_list[rand_num].details, playerChar);
                } else
                    continue;
                ...
                int temp = simulation(old_gameState, nextplayer, 0, depth+1);     //start simulation
                //cout<< number<<" "<<temp<<endl;
                m.lock();   //lock
                rate += temp;       //save win rate
                m.unlock();
            }
            double finish_timer = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
            cout << "finish thread#"<<number<<" : "<<(finish_timer-timer)/1000000000<<endl;
    }

    void SIMULATIONPlannerInterface::three_coin(problem_info& gameState, vector<int>& details, vector<int>& abandon, string& playerChar){  //action get three coin
        custom_msgs::coin& coin_vector = find_coin_vector(playerChar, gameState);
    ...
    void SIMULATIONPlannerInterface::two_coin(problem_info& gameState, vector<int>& details, string& playerChar){  //action get two coin
    ...
    void SIMULATIONPlannerInterface::buy_card(problem_info& gameState, vector<int>& details, string& playerChar){  //action buy card
        custom_msgs::coin& coin_vector = find_coin_vector(playerChar, gameState);
        vector<vector<custom_msgs::card>>& card_vector = find_card_vector(playerChar, gameState);
        ...
        vector<custom_msgs::card>& fold_level_vector = select_vector(get_level_from_int(details[0]), gameState.level1_fold, gameState.level2_fold,
                                                                gameState.level3_fold, gameState.nobility_fold);
        ...
        if(!fold_level_vector.empty()){ //insert random card to field   for simulation
            int rand_num = rand() % fold_level_vector.size();
            level_vector.push_back(fold_level_vector[rand_num]);
            fold_level_vector.erase(fold_level_vector.begin() + rand_num);  //remove card from field
        }
    ...
    std::vector<custom_msgs::card>& SIMULATIONPlannerInterface::select_vector(string n, vector<custom_msgs::card>& level1, vector<custom_msgs::card>& level2, 
    ...
    string SIMULATIONPlannerInterface::get_color_from_int(int n){  //get color string from int(color)
        if (n == 0)
            return "white";
        else if (n == 1)
            return "blue";
    ...
    int& SIMULATIONPlannerInterface::select_color(custom_msgs::card& at, string n) {  //find name and save point
        if (n == "white")
            return at.white;
        else if (n == "blue")
            return at.blue;
    ...
    string SIMULATIONPlannerInterface::get_level_from_int(int n){  //get level string from int(level+nobility) 
        if(n == 0)
            return "level1";
        else if(n==1)
            return "level2";
    ...
    vector<vector<custom_msgs::card>>& SIMULATIONPlannerInterface::find_card_vector(string n, problem_info& p){    //get card vector(player, ai) by string(name)
        if(n == "player1")
    ...
    custom_msgs::coin& SIMULATIONPlannerInterface::find_coin_vector(string n, problem_info& p){    //get coin vector(player, ai) by string(name)
        if(n == "player1")
    ...
    int& SIMULATIONPlannerInterface::select_coin(int n, custom_msgs::coin& p){ //get value of coin' color amount by int(color)
        if(n == 0)
            return p.white;
        else if(n == 1)
            return p.blue;
    ...
    bool SIMULATIONPlannerInterface::check_buy(custom_msgs::coin& p, custom_msgs::card& c, vector<vector<custom_msgs::card>>& card){   //check can buy
    ...
    void SIMULATIONPlannerInterface::make_abandon(custom_msgs::coin coin, action& temp, vector<action>& act_list){ //do abandon
    ...
    void SIMULATIONPlannerInterface::make_plan(string& Msg, problem_info& problem){    //make plan to pdll
    ...
    void SIMULATIONPlannerInterface::make_pddl_from_csp(string& planner_output, std::string path) {    //make file of output
...
int main(int argc, char** argv) {
    ...
    KCL_rosplan::SIMULATIONPlannerInterface pi(nh);

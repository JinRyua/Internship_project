...
int main(int argc,char* argv[]) {
    ...
    if (argc >= 2 && strcmp(argv[1], "test") == 0) {
        ...
        serv_addr.sin_port = htons(PORT);   //port server
        ...
        for(int i =0;i<10;i++) {
        write(clnt_sock,buff.c_str(),buff.size());
        sleep(0);
        }
        ...
        for (int seed = seed_init; seed < seed_end; seed++) {
            for (auto chicha : setup_match_json["chicha"].array_items()) {
                ...
                //game_settings.player_id = 0;
                game_loop(haiyama, game_record, game_settings.chicha, game_settings.player_id, clnt_sock);  //add socket
        ...
        for (int seed = seed_init; seed < seed_end; seed++) {
            ...
            game_loop(haiyama, game_record, game_settings.chicha, game_settings.player_id, -1);
            dump_json_vec_to_file(game_record, match_dir_name + "/haifu_log_" + std::to_string(seed) + "_" + std::to_string(game_settings.chicha) + ".json");
        }
        return 0;
